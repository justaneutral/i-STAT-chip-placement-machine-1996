...
HWND prev_hwnd = 0;
#define IDM_SMAC_TX 32784


void SendCommBlock(LPSTR str, int len)
{				
	int i;
	HWND sendhwnd;
    
	GetSharedMem(256+sizeof(HWND),(LPSTR)&sendhwnd,sizeof(HWND));
	
	for(i=0;str[i] && i<len;i++) 
		PostMessage((HWND)sendhwnd,WM_COMMAND,(WPARAM)IDM_SMAC_TX,(LPARAM)str[i]);
}
...
BOOL Connect(HWND hwnd)
{
   ...
   GetSharedMem(256,(LPSTR)&prev_hwnd,sizeof(HWND));
   SetSharedMem(256,(LPSTR)&hwnd,sizeof(HWND));	
   
   /*
   // Open for read 
   if( (f = fopen( "c:\\pdourbal\\project\\smacdata.txt", "w" )) == NULL )
   {
	   return FALSE;
   }

   fprintf(f,"%d",(unsigned long)hwnd);
...
BOOL Disconnect()
{
   ...
   SetSharedMem(256,(LPSTR)&prev_hwnd,sizeof(HWND));
                    ...
                    LPSTR lpszCmdLine, int nCmdShow )
{
   ...
   //show the hWnd
   sprintf(str0,"hTTYWnd=%d",(unsigned long)hTTYWnd);
   if(!Connect(hTTYWnd))
	 MessageBox(hTTYWnd,str0,"Not registered",MB_ICONINFORMATION);
   else
     MessageBox(hTTYWnd,str0,"Connected",MB_ICONINFORMATION);
      ...
      if ((dwEvtMask & EV_RXCHAR) == EV_RXCHAR)
      {
         do
         {
            if (nLength = ReadCommBlock( hTTYWnd, (LPSTR) abIn, MAXBLOCK ))
            {
               ...
               SendCommBlock((LPSTR) abIn, MAXBLOCK);
				
			   //Update shared memory.
			   SetSharedMem(0,(LPSTR) abIn, nLength);
